import { BookPlus, Check, Copy, Loader2, Sparkles, Star, UserCheck } from "lucide-react";
...
interface NameProposalCardProps {
    ...
    storyId: string;
    // Only passed when this chat is anchored to an existing Codex entry (WorldBuildingChatPanel) —
    // lets the user apply a picked name directly to the entry the chat is already about, instead of
    // "Create Codex entry" always seeding a brand-new entry form. A user click here IS the
    // approval, same posture as "Create Codex entry" being a direct write.
    anchorEntryId?: string;
    anchorEntryName?: string;
}
        ...
        <div className="flex items-center gap-0.5 rounded-full border bg-muted/30 pl-3 pr-1 py-1">
            ...
            {onUseForAnchorEntry && (
                <Button
                    ...
                    size="icon"
                    className="h-6 w-6"
                    title={anchorEntryName ? `Use as ${anchorEntryName}'s name` : "Use as this entry's name"}
                    onClick={onUseForAnchorEntry}
                >
...
export function NameProposalCard({ proposal, storyId, anchorEntryId, anchorEntryName }: NameProposalCardProps) {
    ...
    const handleUseForAnchorEntry = (name: string) => {
        if (!anchorEntryId) return;
        updateLorebookMutation.mutate({ id: anchorEntryId, data: { name } });
    };
                            ...
                            {result?.pairs?.map((pair, index) => {
                                ...
                                return (
                                    <CompactNameChip
                                        ...
                                        onUseForAnchorEntry={anchorEntryId ? () => handleUseForAnchorEntry(fullName) : undefined}
                                        anchorEntryName={anchorEntryName}
                        ...
                        <div className="flex flex-wrap gap-1.5">
                            {result?.names.map((name, index) => (
                                <CompactNameChip
                                    ...
                                    onUseForAnchorEntry={
                                        anchorEntryId && nameType !== "surname" ? () => handleUseForAnchorEntry(name.name) : undefined
                                    }
                                    anchorEntryName={anchorEntryName}
