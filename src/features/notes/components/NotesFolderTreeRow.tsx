...
interface NotesFolderTreeRowProps {
    node: FolderTreeNode;
    depth: number;
...
// Notes' own folder tree row (T7, docs/Notes_Org_Browse_Design.md NO3) — same shape as
// LorebookFolderTreeRow.tsx, duplicated rather than shared since each folder-tree consumer builds
// its own presentational tree over the shared useFoldersQuery/folderTree.ts data layer (see
// ChatFolderNode.tsx's own precedent).
export function NotesFolderTreeRow({
    node,
    depth,
    selectedFolderId,
    onSelect,
    ...
    onMoveTo,
    onDelete
}: NotesFolderTreeRowProps) {
    const [expanded, setExpanded] = useState(true);
    const hasChildren = node.children.length > 0;

    return (
        <div>
            <FolderDropZone id={node.id} data={{ type: "notes-folder", targetFolderId: node.id }}>
                <div
                    className={cn(
