...
Creating effective prompts is essential for getting the best results from AI generation in your story. This guide will help you understand how to create, customise, and use prompts effectively in Story Labyrinth.
...
> [!NOTE]
> **Not the same as Local System Inject.** A desk's **⋯ Edit system prompt** edits the shared prompt *template* for that chat type, for every provider (cloud and local). **Local System Inject** (see that guide tab) is a separate, always-local-only house-rules block prepended on top — and it's the fix if you've set a system prompt in LM Studio's own GUI and found Story Labyrinth ignoring it (LM Studio's preset never reaches SL's API calls).

<Tabs defaultValue="overview" className="w-full">
<TabsList className="grid w-full grid-cols-5 mb-8">
    <TabsTrigger value="overview" data-tab-value="overview">Overview</TabsTrigger>
    <TabsTrigger value="creating" data-tab-value="creating">Creating Prompts</TabsTrigger>
    <TabsTrigger value="variables" data-tab-value="variables">Using Variables</TabsTrigger>
    <TabsTrigger value="managing" data-tab-value="managing">Managing Prompts</TabsTrigger>
    <TabsTrigger value="advanced" data-tab-value="advanced">Advanced Strategies</TabsTrigger>
</TabsList>
...
Story Labyrinth supports various types of prompts to help you with different aspects of your writing process:
...
> [!NOTE]
> **Proposal fences aren't prompt variables.** Several chat types (World-Building, Outline, Story Timeline, Notes, Brainstorm) have their own system-prompt instructions telling the model to emit structured proposal blocks — Lore Sheet drafts, place-sheet fields, psych profiles, timeline pins, note splits, chat handoffs, and more — which render as Accept/Reject cards instead of plain text. These are separate from the `{{variable}}` system on this page: they're baked into that chat type's own framing, not something you write into a custom prompt yourself.
