...
12. **P1.3 done** (2026-07-23) — per-message RAG re-seeding for World-Building/Editor/Outline chats (Research/Brainstorm/Notes excluded, each for its own existing reason). On each send, `ChatInterface.tsx` re-fetches chat context using the current outgoing message as the RAG query (same precedent as Research's `explicitQuery`), keeps only `role: "search"` results (anchor/related content stays unconditional and unchanged), and merges them into the prompt as `extraContext` alongside the existing mount-time title-seeded context — no server changes needed, `GET /:chatId/context?query=...` already supported this shape. See `DECISIONS.md`'s "P1.3 — Per-Message RAG Re-Seeding" entry. Remaining P1: P1.4 (Visible AI Reasoning full product, still needs scope confirmation). Otherwise P2 bugs or promote a P3 item as needed.
