...
### Story Timeline
- Design: `docs/Story_Timeline_Design.md` (design locked 2026-08-06) — **TL0-TL4 core foundation implemented (2026-08-07)**, TL5-TL12 still open à la carte
...
- Schema: `storyTimelines` (spine + future named ones, per-timeline `orientation`, Story-start anchor config), `storyTimelinePins` (story-scoped SoT — a pin's existence is independent of which timeline(s) show it), `storyTimelineMemberships` (join, `laneId` schema-only for future swimlanes). Spine is **lazy get-or-create** (`ensureSpineTimeline`, runs on first fetch) rather than hooked to story creation, so every story — new or pre-existing — gets one
...
- Story-start defaults to Chapter One (pre-filled from the lowest-order chapter), always manually overridable to a specific pin or a freeform manual time (`StoryStartControl.tsx`)
...
- Story Timeline tool ("Timeline" in the workspace sidebar, peer of Maps) — H\|V orientation toggle per-timeline, Story-start marker chrome
- **Not yet built:** named timelines + board switcher (TL5), swimlanes UI (TL6), AI propose/accept (TL7), opt-in chat chronology context (TL8), era/overview strip (TL9), export image (TL10), scanner hook + bulk suggest (TL11), Guide + polish (TL12)
