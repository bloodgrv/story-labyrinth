...
2. **P0.4 R0-R8 + B0-B5 done** (2026-07-20) — Editor selection rework + Codex tray (R0-R3); Lorebook field rework → WB chat, dedicated Outline chat with its own `chatType` (R4/R5/R7), outline row rework + "Open in WB" lore-suggestion handoff (R8); auto-insert/auto-accept toggles per host (R6); Brainstorm Hub migrated onto the shared chat stack with Guided Setup, depth-adaptive propose, and a durable handoff/checklist tray (B0-B4); WB + Outline guided-start extended with the same shell, plus WB's Character psych module (B5). Remaining: the Research/Notes-desk (S/K) tracks
