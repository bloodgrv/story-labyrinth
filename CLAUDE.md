...
13. **Locations & Maps L0-L3 done** (2026-07-22) — see `docs/Locations_And_Maps_Design.md` and `DECISIONS.md`'s "Locations & Maps — L0-L3" entry. Location-focused WB grill (10-slot script, prompt-driven); light editable "place sheet" on location entries (`metadata.placeState` — scale/biome/holder/danger/landmarks/exits/layoutMd/imageBrief), both manually editable on the entry form and chat-propose→accept via a new `place-sheet-proposal` fence (same posture as the Character psych module); a "Map" image-generation preset alongside the existing "Mood" one, location-only; a new **Story Map** tool (sidebar) — its own `storyMapEdges`/`storyMapLayout` tables and spatial edge-type vocabulary (`contains`/`borders`/`road_to`/`portal_to`/`below`/`above`/`other`, deliberately separate from the Relationship Graph's own 15-value factual allowlist), manual-CRUD-only this pass (no pending/AI-suggest lane yet, mirrors how the Relationship Graph itself staged G1 before G1.5+). Found and fixed a real pre-existing bug along the way: `entryFormUtils.ts`'s `buildSubmitData` hardcoded the `metadata` object on every normal entry save, wholesale-replacing (not merging) the column — silently wiping `psychProfile` (and now `placeState`) on any plain "edit description, hit Save." L4 (full place-Codex versioning) and L5 (travel-time/multi-floor/export) not started.
