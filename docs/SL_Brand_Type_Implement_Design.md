...
**Status:** **Design locked 2026-08-04** — **BT0-BT4 all shipped 2026-08-04**. See `DECISIONS.md`'s "Story Labyrinth Brand & Type — BT0-BT2" and "— BT3-BT4" entries.  
...
| **BT3** | ✅ **Done 2026-08-04.** Brand CSS tokens — `--brand-crimson`, `--brand-ember` added once to the true `:root` (theme-independent, not duplicated per theme block since none of the 18 theme classes override them). One restrained call site: hover-only ember glow on the TopBar wordmark image. **No** global primary repaint. | Tokens exist (confirmed live via `getComputedStyle`); exactly 1 restrained call site; menus not crimson. Live hover-simulation check was inconclusive in this session's Browser-pane sandbox specifically (compiled rule, token values, and an identical inline-applied declaration all verified correct — see DECISIONS.md) |
| **BT4** | ✅ **Done 2026-08-04.** About blurb — no dedicated About/Help page exists; added a one-line credit + freeware posture to the Guide page's existing welcome header instead (the closest qualifying surface), "Story Labyrinth" in `font-display` (Cormorant Garamond) | No drive-by string replace — existing Guide title/description left unchanged, new line added below |
