...
- **2026-08-12** — Post-ship follow-up, user-flagged: WB's older, pre-T10 `ContextSelector` ("Story Context" — Include Full Context / Chapter Summaries / Chapter Content / Lorebook Entries, a genuinely separate mechanism from the Context & memory toggle bucket this doc's CR4 covers) was never in this doc's scope and had been left rendering inline in the main chat column after CR7's WB migration, sitting redundantly next to the rail's own "Context" icon. Folded into `ChatToolsRail` as a new "Story Context" bucket (`Library` icon) using the exact CR4 lifted-state pattern (`useContextSelection()` called once in `WorldBuildingChatPanel`, passed to `ChatInterface` via new `contextSelection`/`storyContextPanelMode="external"` props, mirroring `contextToggles`/`contextPanelMode`). `ContextSelector.tsx` gained an optional `hideHeader` prop so its own repeated "Story Context" label + nested collapse toggle don't duplicate the rail Sheet's own title, same "content-only" shape `ChatContextPanelContent.tsx` already established. WB-only (this selector has always been WB-only, gated on `showContextSelector`). `npx tsc --noEmit` and `npm run build` clean; live-verified in the Browser pane: rail shows the new icon between Context and Playbook, opens a Sheet titled "Story Context" with no duplicate header, a Chapter Summary toggle flips correctly, and the closed rail icon's badge (dot when collapsed, "N items"/"Full context" text when expanded) tracks the armed state live.
