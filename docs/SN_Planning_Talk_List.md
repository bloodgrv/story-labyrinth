...
**Updated:** 2026-07-21 (T2 plugin-add locked to fork doc)
...
| **T3** | **Amazon text / KDP standards** | Manuscript & export aligned with Amazon KDP / Kindle text expectations (and related trade rules worth caring about): front matter, paragraph/indent, fonts in export, scene breaks, image rules, HTML/EPUB/PDF quirks, “looks right on Kindle” checklist vs our export paths. | **To discuss** |
| **T4** | **Context / token meter** | Local used-vs-left (+ usage-capable). Doc: `docs/Context_Token_Meter_Design.md`. M0–M5. | **Locked** (P3, not started) |
...
1. Offer **T2 remaining axes** + **T3** + P1 + design-locked P3 (Outline Import, Transfer log/Settings IA, Name gen, Locations, T4 meter, Lexical LE0–LE3)  
2. One topic at a time; lock to `docs/` when decided  
3. Don’t start Lexical **build** until promoted / user says build — plugin-add design is locked; don’t re-grill List/CheckList/tables-C unless reopened  
4. Don’t start Amazon/token-meter builds until those items are locked **and** promoted (T4 already design-locked)
