...
**Status:** Findings. **Wave 0 (H2 + H3 Tiers 1–2) shipped the same day** — see B46/B47 in `docs/CURRENT_BACKLOG.md` and the "Wave 0 — Canvas Removal + Dependency Wave" entry in `DECISIONS.md`. Everything else here is still open. The findings below are preserved as written on the day of the sweep; they are *not* rewritten as work lands, so read them as a snapshot and trust the backlog for current state.
